...
*******************************************************************************/
void ICACHE_FLASH_ATTR
uart_init(UartBautRate uart0_br,UartBitsNum4Char uart0_db,UartParityMode uart0_pm, UartStopBitsNum uart0_sb
		, UartBautRate uart1_br)
{
  // rom use 74880 baut_rate, here reinitialize
  UartDev.baut_rate = uart0_br;
  UartDev.data_bits = uart0_db;
  UartDev.parity = uart0_pm;
  UartDev.stop_bits = uart0_sb;
  uart_config(UART0);
...
uart_reattach()
{
  uart_init(BIT_RATE_74880,EIGHT_BITS,NONE_BITS,ONE_STOP_BIT, BIT_RATE_74880);
//  ETS_UART_INTR_ATTACH(uart_rx_intr_handler_ssc,  &(UartDev.rcv_buff));
//  ETS_UART_INTR_ENABLE();
}

void ICACHE_FLASH_ATTR
uart0_SetWordLength( UartBitsNum4Char len)
{
    SET_PERI_REG_BITS(UART_CONF0(UART0),UART_BIT_NUM,len,UART_BIT_NUM_S);
}

void ICACHE_FLASH_ATTR
uart0_SetStopBits(UartStopBitsNum bit_num)
{
    SET_PERI_REG_BITS(UART_CONF0(UART0),UART_STOP_BIT_NUM,bit_num,UART_STOP_BIT_NUM_S);
}



void ICACHE_FLASH_ATTR
uart0_SetParity(UartParityMode Parity_mode)
{
    CLEAR_PERI_REG_MASK(UART_CONF0(UART0), UART_PARITY |UART_PARITY_EN);
    if(Parity_mode==NONE_BITS){
    }else{
        SET_PERI_REG_MASK(UART_CONF0(UART0), Parity_mode|UART_PARITY_EN);
    }
}
